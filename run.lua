...
uart.write(0,0x10,0x02,0x01,0xD3,0x10,0x03,0xC1)
...
function vfdsend(cmd,disp)
    ...
    uart.write(0,0x10,0x02,#disp+1,cmd,disp,0x10,0x03,x1)
    --uart.setup(0, 115200, 8, uart.PARITY_NONE, uart.STOPBITS_1, 1 )
end

local function human(level)
    gpio.write(0, gpio.LOW)    -- LED ON
    tmr.stop(1)
    tmr.start(1)        -- restart tm1
end

gpio.trig(8,"high",human)     -- star check human

tmr.alarm(1, 60000, tmr.ALARM_SEMI, function()
        gpio.write(0, gpio.HIGH)    -- VFD OFF
end)
...
tmr.alarm(0, 2000, tmr.ALARM_AUTO, function()
         ...
         --print(string.char(tm["hour"])..string.char(tm["min"]))
         vfdsend(0x3A,string.char(1)..string.char(tmhour)..string.char(tmmin)..string.char(4))
